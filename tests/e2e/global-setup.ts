...
async function globalSetup(_config: FullConfig) {
  ...
  // Check if admin.json exists and is fresh enough
  if (fs.existsSync(authFile)) {
    const fileAge = formatFileAge(authFile);
    console.log(`📁 Admin session found (antigüedad: ${fileAge})`);

    if (isAuthFileOlderThan(authFile, 3)) {
      console.log('⚠️  Admin session is older than 3 hours - regenerating...\n');
    } else {
      console.log('✅ Admin session is fresh - reusing existing session');
      console.log('💡 Tests will reuse the existing session - no login overhead!\n');
      console.log('🎯 E2E Global Setup complete\n');
      return;
    }
  } else {
    console.log('⚠️  Admin session not found - generating via API...\n');
  }

  console.log('⚠️  Admin session not found - generating via API...\n');

  // Remove old auth file if it exists but is too old
  if (fs.existsSync(authFile)) {
    try {
      fs.unlinkSync(authFile);
      console.log('🗑️  Removed old admin session file');
    } catch (error) {
      console.warn('⚠️  Could not remove old auth file:', error);
    }
  }
