...
test.describe('Story 1.3: Etiquetas de Clasificación y Organización (ATDD - RED PHASE)', () => {
  ...
  test('[P0-E2E-001] should not grant capabilities when tag is assigned', async ({ page, request }) => {
    ...
    // Wait for the tag checkbox to be visible with a longer timeout
    // Use a more specific wait with better error handling
    try {
      await page.getByTestId('tag-checkbox-Supervisor').waitFor({ state: 'visible', timeout: 10000 });
    } catch (e) {
      // If checkbox not found, take screenshot for debugging
      await page.screenshot({ path: 'debug-tag-checkbox-not-found.png' });
      console.log('[DEBUG] Tag checkbox not found, checking if tags section exists...');

      // Check if tags section exists at all
      const tagsSection = page.getByTestId('tags-section').or(page.getByText(/Etiquetas/i)).or(page.getByText(/Tags/i));
      if (await tagsSection.count() > 0) {
        console.log('[DEBUG] Tags section found but checkbox not available');
        // List available testids for debugging
        const testIds = await page.locator('[data-testid]').all();
        console.log('[DEBUG] Available testids:', testIds.map(async (el) => await el.getAttribute('data-testid')).slice(0, 20));
      } else {
        console.log('[DEBUG] Tags section not found at all - feature may not be implemented');
      }
      throw e;
    }
    ...
    // Look for save button - try multiple possible selectors
    const saveButton = page.getByRole('button', { name: 'Guardar Etiquetas' })
      .or(page.getByRole('button', { name: /Guardar/i }))
      .or(page.getByTestId('save-tags-button'))
      .or(page.getByTestId('tag-save-button'))
      .or(page.getByTestId('save-button'))
      .first();

    await saveButton.click();
    await page.waitForTimeout(500); // Brief pause to let save complete
