...
export function TagList({ tags }: TagListProps) {
  ...
  const handleDelete = async (tagId: string, tagName: string) => {
    // Confirmation dialog with clarifier message (AC5 requirement)
    const confirmationMessage = `¿Eliminar etiqueta "${tagName}"? Esta acción no afecta las capabilities de los usuarios.`
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Error al eliminar etiqueta')
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {tags.map((tag) => (
          ...
          >
            ...
            {/* Info message if tag has users */}
            {(tag._count?.userTags || 0) > 0 && (
              <p className="mt-2 text-xs text-gray-500">
                Esta etiqueta está asignada a {(tag._count?.userTags || 0)} {(tag._count?.userTags || 0) === 1 ? 'usuario' : 'usuarios'}
              </p>
            )}
          </div>
        ))}
